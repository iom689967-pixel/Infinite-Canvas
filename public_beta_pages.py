...
def page(kind,config):
    title={'home':'Mio Canvas','login':'登录 Mio Canvas','register':'注册 Mio Canvas',
           'verify-email':'验证 Mio Canvas 邮箱','resend-verification':'重新发送验证邮件'}[kind]
    script=''
    ...
    if kind=='home':
        ...
        body='<form>'+email+'<button type="submit">重新发送验证邮件</button></form><p><a href="/login">返回登录</a></p>'
    else:
        ...
        if kind=='register':
            body='<p>Public Beta · 免费使用<br>API 由用户自行配置</p><p>Public Beta 当前开放少量测试名额。</p><form>'+email
            body+='<label>用户名 / 昵称<input name="username" minlength="3" maxlength="32" pattern="[A-Za-z0-9][A-Za-z0-9_-]{2,31}" autocomplete="nickname" required></label>'+password
            ...
            body+='<button type="submit">注册</button></form><p><a href="/login">已有账号，登录</a><a href="/resend-verification">重新发送验证邮件</a></p>'
        else:
            ...
            body+='<a href="/resend-verification">重新发送验证邮件</a></p>'
    if kind in {'login','register','resend-verification'} and not (kind=='register' and config.registration_mode=='closed'):
        endpoint={'login':'login','register':'register','resend-verification':'resend-verification'}[kind]
