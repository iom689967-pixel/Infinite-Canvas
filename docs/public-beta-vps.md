...
本次发布只更新开发分支与 `beta`，不经过或修改 `main` / `stable`。
上线前实测注册已为 `open`、已有 1 个有效用户；不得按旧说明把现有数据清空。
Public Beta 新注册密码至少 12 字符，确认密码使用相同规则，服务端同步检查。
既有账号登录不重新应用密码创建下限；本地单机账号规则独立，owner 不受影响。
注册页说明免费使用、API 由用户自行配置以及测试名额有限。
